-- =====================================================
-- ITII Assist Classroom - Database Structure (Documented)
-- =====================================================
-- วันที่สร้าง: 17 มกราคม 2569
-- 
-- สถานะการใช้งาน:
--   ✅ ACTIVE     = ใช้งานอยู่ มี Model รองรับ
--   ⚠️ PARTIAL   = ใช้บางส่วน หรือเตรียมไว้ใช้
--   ❌ UNUSED    = ยังไม่มี Model รองรับ (อาจลบได้)
--   🔧 LEGACY    = เก็บไว้เพื่อ backward compatibility
-- =====================================================
...
CREATE TABLE `courses` (
  ...
  `instructor_id` bigint DEFAULT NULL COMMENT '🔧 LEGACY: FK->users อาจารย์หลัก (ใช้ course_instructors แทน)',
...
CREATE TABLE `assignments` (
  ...
  `linked_attendance_session_id` bigint DEFAULT NULL COMMENT '🔧 LEGACY: FK->attendance_sessions (ใช้ assignment_attendance_links แทน)',
...
-- สถานะ: ✅ ACTIVE
-- Model: AssignmentAttendanceLink.js
...
-- สถานะ: ✅ ACTIVE
-- Model: AttendanceSession.js
-- คำอธิบาย: Session สำหรับเช็คชื่อนักศึกษา
-- =====================================================
CREATE TABLE `attendance_sessions` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
  `course_id` varchar(21) NOT NULL COMMENT 'FK->courses: รหัสรายวิชา',
  `course_section_id` bigint DEFAULT NULL COMMENT '🔧 LEGACY: FK->course_sections (ใช้ attendance_session_sections แทน)',
  ...
  -- ❌ UNUSED: `duration_minutes` int - ลบแล้ว ไม่ได้ใช้งาน
...
-- สถานะ: ✅ ACTIVE
-- Model: AttendanceSessionSection.js
...
CREATE TABLE `attendance_records` (
  ...
  -- ❌ UNUSED: `timestamp` datetime - ลบแล้ว ใช้ check_in_time แทน
  ...
  -- ❌ UNUSED: `sso_identifier` varchar(255) - ลบแล้ว ไม่ได้ใช้งาน
  -- ❌ UNUSED: `verified` tinyint(1) - ลบแล้ว ซ้ำซ้อนกับ pin_verified + location_verified
...
-- =====================================================
-- =====================================================
-- ตารางที่ยังไม่มี Model รองรับ (❌ UNUSED)
-- อาจพิจารณาลบหรือสร้าง Model เพิ่ม
-- =====================================================
-- =====================================================

-- =====================================================
-- ตาราง: queue_sessions (Session คิวตรวจงาน)
-- สถานะ: ❌ UNUSED - ยังไม่มี Model
-- คำอธิบาย: ระบบจัดคิวตรวจงานในห้องปฏิบัติการ
--           เตรียมไว้สำหรับฟีเจอร์ในอนาคต
-- =====================================================
...
-- =====================================================
-- 
-- ✅ ACTIVE (มี Model รองรับ - 23 ตาราง):
-- 1.  users                        - ผู้ใช้ระบบ
-- 2.  students                     - นักศึกษา
...
-- 11. assignments                  - งานมอบหมาย
-- 12. assignment_sub_items         - หัวข้อย่อยของงาน
-- 13. assignment_attendance_links  - เชื่อมงานกับ session เช็คชื่อ
-- 14. scores                       - คะแนน
-- 15. score_edit_requests          - คำขอแก้ไขคะแนน
-- 16. bonus_scores                 - คะแนนพิเศษ
-- 17. attendance_sessions          - Session เช็คชื่อ
-- 18. attendance_session_sections  - Section ของ Session เช็คชื่อ
-- 19. attendance_records           - บันทึกการเช็คชื่อ
-- 20. classrooms                   - ห้องเรียน
-- 21. desks                        - โต๊ะในห้องเรียน
-- 22. feedbacks                    - Feedback/รายงานปัญหา
-- 23. system_logs                  - Log ระบบ
--
-- ❌ UNUSED TABLES (ยังไม่มี Model - 4 ตาราง):
-- 1.  queue_sessions               - Session คิวตรวจงาน
-- 2.  queue_bookings               - การจองคิว
-- 3.  queue_desk_status            - สถานะโต๊ะในคิว
-- 4.  queue_workers                - ผู้ตรวจงานในคิว
--
-- ❌ UNUSED COLUMNS (ควรลบออก - 6 คอลัมน์):
-- 1.  users.linked_student_id           - ลบแล้ว ไม่ได้ใช้งาน
-- 2.  attendance_records.timestamp      - deprecated ใช้ check_in_time แทน
-- 3.  attendance_records.sso_identifier - ไม่ได้ใช้ใน Model
-- 4.  attendance_records.verified       - ซ้ำซ้อนกับ pin_verified + location_verified
...
-- - courses.instructor_id          - ใช้ course_instructors แทน
-- - assignments.linked_attendance_session_id - ใช้ assignment_attendance_links แทน
-- - attendance_sessions.course_section_id - ใช้ attendance_session_sections แทน
--
-- 📝 Migration Script: 010_cleanup_unused_columns.sql
-- =====================================================
