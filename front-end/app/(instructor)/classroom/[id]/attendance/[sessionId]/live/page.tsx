...
export default function LiveAttendancePage() {
  ...
  const socket = io(window.location.origin, {
    ...
    transports: ["websocket"],
        ...
        socket.on("connect", () => {
            console.log("Socket connected, joining room: instructor-" + sessionId);
            // Join instructor room - use string to match backend
            socket.emit("join-instructor", String(sessionId));
        });
        ...
        return () => {
            socket.emit("leave-instructor", String(sessionId));
            socket.disconnect();
        };
    }, [sessionId]);
                    ...
                    <Modal isOpen={isQRModalOpen} onClose={() => setIsQRModalOpen(false)} size="full">
                        <ModalContent className="bg-white">
                            <ModalBody className="flex flex-col items-center justify-center min-h-screen py-10">
                                ...
                                <div className="p-6 bg-white rounded-3xl shadow-xl border-4 border-slate-100">
                                    <QRCodeSVG 
                                        value={checkInUrl} 
                                        size={350} 
                                        level="L" 
